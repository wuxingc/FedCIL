...
from utilstools.inc_net import IncrementalNet
from methods.base import BaseLearner
from utilstools.data_manager import partition_data, DatasetSplit, average_weights, setup_seed, pil_loader
from utilstools.bmr_diagnostics import BoundaryMarginRiskDiagnostics  # 导入简化版BMR诊断工具
import copy
import os, math
import pickle
...
class IndexedDataset(Dataset):
    def __init__(self, dataset, indices, transform):
        ...
        # tiny_imagenet stores file paths (string dtype) instead of raw pixel arrays
        self.use_path = np.issubdtype(self.images.dtype, np.str_)
...
class BBBB(BaseLearner):
    def __init__(self, args):
        ...
        self.class_order = torch.tensor(args["class_order"], device="cuda:0")
        self.args = args
        ...
        # 初始化BMR诊断工具（简化版：单一边界风险指标）
        self.bes_diagnostics = BoundaryMarginRiskDiagnostics(
            self,
            output_dir="run/bmr_diagnostics",
            tau=0.2  # 20%分位数
        )
        print("=" * 80)
        print("✅ Using Boundary Margin Risk (BMR) Diagnostics - Simplified Version")
        print("   Single metric: Low-tail Old-New Boundary Margin")
        print("   Output directory: run/bmr_diagnostics/")
        print("=" * 80)
...
#         ipdb.set_trace()
        ...
        # === BMR诊断：在数据选择前计算边界风险 ===
        print("\n" + "="*60)
        print(f"Computing BMR for Task {self._cur_task}...")
        print("="*60)
        self.bes_diagnostics.compute_bmr_for_task(self._cur_task)
...
#                 ipdb.set_trace()
                ...
                loss_bmr = boundary_margin_loss(
                    output,
                    labels,
                    self._known_classes,
                    margin=self.args["bmr_margin"]
                )

                loss = loss_ce + self.args["lambda_bmr"] * loss_bmr
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def boundary_margin_loss(logits, labels, num_old_classes, margin=0.5):
    if num_old_classes <= 0 or logits.size(1) <= num_old_classes:
        return logits.new_tensor(0.0)

    old_mask = labels < num_old_classes
    if old_mask.sum() == 0:
        return logits.new_tensor(0.0)
    ...
    true_old_logit = old_logits.gather(1, old_labels.view(-1, 1)).squeeze(1)
    max_new_logit = old_logits[:, num_old_classes:].max(dim=1).values

    old_new_margin = true_old_logit - max_new_logit
    return F.relu(margin - old_new_margin).mean()
