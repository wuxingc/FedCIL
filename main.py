import argparse
import os
from utilstools.data_manager import DataManager, setup_seed
from utilstools.toolkit import count_parameters
...
def get_learner(model_name, args):
    ...
    logging.basicConfig(
        ...
        + "_local_epoch_"
        + str(args["local_ep"])
        + "_experiment_log-%s" % (datetime.datetime.now().strftime("%Y-%m-%d-%H%M-%S"))
        + ".log",  # 日志输出到文件
        filemode="a",  # 追加模式
    )
...
def train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        cnn_accy, nme_accy = learner.eval_task(
            args, task * data_manager.get_task_size(task)
        )
        learner.after_task()
...
def args_parser():
    parser = argparse.ArgumentParser(
        description="benchmark for federated continual learning"
    )
    # Exp settings
    parser.add_argument(
        "--exp_name", type=str, default="lander_b0", help="name of this experiment"
    )
    ...
    parser.add_argument(
        "--beta", type=float, default=0.5, help="control the degree of label skew"
    )
    parser.add_argument(
        "--frac", type=float, default=1.0, help="the fraction of selected clients"
    )
    parser.add_argument("--class_shuffle", type=int, default=1, help="class shuffle")

    # Data-free Generation
    parser.add_argument(
        "--lr_g", default=2e-3, type=float, help="learning rate of generator"
    )
    parser.add_argument(
        "--synthesis_batch_size",
        default=256,
        type=int,
        help="synthetic data batch size",
    )
    parser.add_argument(
        "--bn", default=1.0, type=float, help="parameter for batchnorm regularization"
    )
    parser.add_argument(
        "--oh", default=0.5, type=float, help="parameter for similarity"
    )
    parser.add_argument(
        "--adv", default=1.0, type=float, help="parameter for diversity"
    )
    parser.add_argument(
        "--nz", default=256, type=int, help="output size of noisy nayer"
    )
    parser.add_argument(
        "--nums", type=int, default=10000, help="the num of synthetic data"
    )
    parser.add_argument(
        "--warmup",
        default=10,
    ...
    parser.add_argument(
        "--syn_round", default=20, type=int, help="number of synthetize round."
    )
    parser.add_argument(
        "--g_steps", default=40, type=int, help="number of generation steps."
    )

    # Client Training
    parser.add_argument(
        "--num_worker", type=int, default=4, help="number of worker for dataloader"
    )
    parser.add_argument(
        "--mulc", type=str, default="fork", help="type of multi process for dataloader"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--local_lr", default=4e-2, type=float, help="learning rate for optimizer"
    )
    parser.add_argument("--kd", default=0.1, type=float, help="weight for kd")
    parser.add_argument(
        "--tau_old", default=0.5, type=float, help="temperature parameter"
    )
    parser.add_argument(
        "--tau_new", default=0.5, type=float, help="temperature parameter"
    )
    ...
    parser.add_argument("--lambda_bmr", default=0.1, type=float)
    parser.add_argument("--bmr_margin", default=0.5, type=float)
    ...
    parser.add_argument(
        "--ltc", default=5, type=float, help="lamda_ltc parameter for LTE center"
    )
    parser.add_argument(
        "--pre",
        type=float,
        default=0.4,
        help="alpha_pre for distilling from previous task",
    )
    parser.add_argument(
        "--cur", type=float, default=0.2, help="alpha_cur for current task training"
    )
    parser.add_argument(
        "--type", default=-1, type=int, help="seed for initializing training."
    )  # 0 for train forward, 1 pretrain stage 1, 2 pretrain stage 2
    parser.add_argument(
        "--syn", default=1, type=int, help="seed for initializing training."
    )  # 0 for train forward, 1 pretrain stage 1, 2 pretrain stage 2
